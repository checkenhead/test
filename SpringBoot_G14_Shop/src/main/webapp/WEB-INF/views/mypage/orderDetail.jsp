...
<form name="orderDetailFrm" method="post">
   <h3> 주문자 정보 </h3>
   <table id="cartList">
   ...
   <table id="cartList">
      <tr><th>상품명</th><th>상품별주문번호</th> <th>수량</th><th>가격</th><th>처리 상태</th></tr>
       <c:forEach items="${orderList}"  var="orderVO">
          <tr><td> ${orderVO.PNAME} </td><td> ${orderVO.ODSEQ}</td><td> ${orderVO.QTY} </td>
            <td> <fmt:formatNumber type="currency"    value="${orderVO.PRICE2*orderVO.QTY}" /></td>
            <td>
               ...
               <c:choose>
                  ...
                  <c:when test='${orderVO.RESULT=="3"}'>
                     ...
                     onClick="go_order_end('${orderVO.OSEQ}', '${orderVO.ODSEQ}');">
                  </c:when>
